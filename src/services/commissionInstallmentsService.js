 ...
 */
export function fetchInstallments(filters = {}) {
  ...
  if (filters.developer_name) {
    data = data.filter(d => d.developer_name === filters.developer_name);
  }
  if (filters.deal_id) {
    data = data.filter(d => d.deal_id === filters.deal_id);
  }
 ...
 */
export function updateInstallment(id, updates) {
  const all = loadAll();
 ...
 * Get aggregate stats
 */
export function getInstallmentStats() {
  const data = autoMarkOverdue(loadAll());
