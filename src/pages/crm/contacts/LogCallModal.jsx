...
export default function LogCallModal({ contact, onClose }) {
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      width="max-w-[420px]"
      ariaLabel={isRTL ? 'تسجيل مكالمة' : 'Log Call'}
      title={<span className="flex items-center gap-1.5"><Phone size={14} /> {isRTL ? 'تسجيل مكالمة' : 'Log Call'} — {contact.full_name}</span>}
    >
      {/* Call Result */}
      ...
      <ModalFooter className="justify-end">
        ...
        <Button onClick={handleSave} loading={saving} disabled={!callResult}>
          {addFollowup ? (isRTL ? 'حفظ + إنشاء مهمة' : 'Save + Create Task') : (isRTL ? 'حفظ المكالمة' : 'Save Call')}
        </Button>
      </ModalFooter>
    </Modal>
  );
