      ...
      <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
        ...
        {activeTab === 'campaigns' && (
          <div className="flex gap-2">
            <ExportButton
              data={filtered.map(c => ({ name: isRTL ? c.name_ar : c.name_en, platform: c.platform, status: c.status, type: c.type, budget: c.budget, spent: c.spent, leads: campaignStats[c.id]?.leads || 0, cpl: campaignStats[c.id]?.cpl || 0, start: c.start_date, end: c.end_date }))}
              filename="campaigns"
              title={isRTL ? 'الحملات' : 'Campaigns'}
              columns={[{ header: 'Name', key: 'name' }, { header: 'Platform', key: 'platform' }, { header: 'Status', key: 'status' }, { header: 'Budget', key: 'budget' }, { header: 'Spent', key: 'spent' }, { header: 'Leads', key: 'leads' }, { header: 'CPL', key: 'cpl' }]}
            />
