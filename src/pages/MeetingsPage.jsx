...
import { MapPin, Building2, Video, Clock, CalendarClock, CheckCircle2, ChevronLeft, Search, MessageSquare } from 'lucide-react';
...
export default function MeetingsPage() {
  ...
  const toAr = useCallback((n) => (isRTL ? String(n).replace(/\d/g, d => AR_DIG[+d]) : String(n)), [isRTL]);

  const load = useCallback(async () => {
    if (!profile?.id) return;
    setLoading(true);
    try {
      const data = await fetchActivities({ type: 'meeting', role: profile.role, userId: profile.id, teamId: profile.team_id, limit: FETCH_LIMIT });
      const arr = Array.isArray(data) ? data : [];
      // One retry for a transient empty result (session token still warming up
      // on a cold navigation) — avoids a false "No meetings" flash.
      if (arr.length === 0 && !retriedRef.current) {
        retriedRef.current = true;
        setTimeout(() => loadRef.current?.(), 900);
        return;
      }
      setMeetings(arr);
    } catch { setMeetings([]); }
  ...
  const filtered = useMemo(() => {
    ...
    return [...list].sort((a, b) => {
      ...
      if (tab === 'upcoming') {
        ...
        if (ao !== bo) return ao ? 1 : -1;      // real upcoming first, overdue after
  ...
  const timeParts = (m) => {
    const s = whenOf(m).toLocaleTimeString(isRTL ? 'ar-EG' : 'en-US', { hour: '2-digit', minute: '2-digit' });
    const parts = s.trim().split(/\s+/);
    return parts.length >= 2 ? { hm: parts.slice(0, -1).join(' '), ap: parts[parts.length - 1] } : { hm: s, ap: '' };
  };
    ...
    <div dir={isRTL ? 'rtl' : 'ltr'} className="min-h-full bg-surface-bg dark:bg-surface-bg-dark">
      <div className="max-w-[860px] mx-auto px-4 md:px-7 pt-5 md:pt-7 pb-24">
        ...
        <div className="grid grid-cols-3 gap-2.5 md:gap-3 mb-5">
          {KPIS.map((k, i) => (
            <div key={i} className="bg-surface-card dark:bg-surface-card-dark border border-edge dark:border-edge-dark rounded-2xl p-3.5 md:p-4 shadow-[0_1px_2px_rgba(20,30,50,0.04)]">
              ...
              <div className="text-[24px] leading-none font-extrabold text-content dark:text-content-dark tabular-nums">{toAr(k.value)}</div>
          ...
          <div className="inline-flex p-1 rounded-2xl bg-surface-card dark:bg-surface-card-dark border border-edge dark:border-edge-dark shadow-[0_1px_2px_rgba(20,30,50,0.04)]">
            {TABS.map(t => (
              <button key={t.k} onClick={() => setTab(t.k)}
                className={`px-3.5 py-1.5 rounded-xl text-[12.5px] font-bold cursor-pointer border-none transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/40 ${tab === t.k ? 'bg-brand-500 text-white shadow-[0_2px_8px_rgba(47,107,211,0.30)]' : 'bg-transparent text-content-muted dark:text-content-muted-dark hover:text-content dark:hover:text-content-dark'}`}>
                {isRTL ? t.ar : t.en}<span className={`ms-1.5 text-[10px] ${tab === t.k ? 'opacity-80' : 'opacity-60'}`}>{toAr(t.n)}</span>
              </button>
            ))}
          </div>
          ...
          {[{ k: 'all', ar: 'كل الأنواع', en: 'All types', color: '#6B7688' }, ...Object.entries(SUBTYPES).map(([k, v]) => ({ k, ar: v.ar, en: v.en, color: v.color, Icon: v.Icon }))].map(t => {
            ...
            return (
              <button key={t.k} onClick={() => setTypeFilter(t.k)}
                className={`px-3 py-1.5 rounded-full text-[11.5px] font-bold cursor-pointer border inline-flex items-center gap-1.5 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-500/40 ${on ? '' : 'bg-transparent border-edge dark:border-edge-dark text-content-muted dark:text-content-muted-dark hover:text-content dark:hover:text-content-dark'}`}
        ...
        {busy ? (
          <div className="space-y-3">
            {[0, 1, 2, 3].map(i => <div key={i} className="h-[78px] rounded-2xl bg-surface-card dark:bg-surface-card-dark border border-edge dark:border-edge-dark animate-pulse" />)}
          </div>
        ) : groups.length === 0 ? (
          ...
          <div className="space-y-7">
              ...
              <div key={gi}>
                ...
                <div className="flex items-center gap-3 mb-3">
                  <div className={`flex flex-col items-center justify-center w-11 h-11 rounded-2xl shrink-0 ${isToday(g.date) ? 'bg-brand-500 text-white' : 'bg-surface-card dark:bg-surface-card-dark border border-edge dark:border-edge-dark text-content dark:text-content-dark'}`}>
                    <span className="text-[15px] font-extrabold leading-none tabular-nums">{toAr(g.date.getDate())}</span>
                    <span className={`text-[8.5px] font-bold mt-0.5 ${isToday(g.date) ? 'text-white/80' : 'text-content-muted dark:text-content-muted-dark'}`}>{g.date.toLocaleDateString(isRTL ? 'ar-EG' : 'en-US', { month: 'short' })}</span>
                  </div>
                  <div>
                    <div className="text-[13.5px] font-extrabold text-content dark:text-content-dark leading-tight">{dayLabel(g.date)}</div>
                    <div className="text-[11px] text-content-muted dark:text-content-muted-dark">{g.date.toLocaleDateString(isRTL ? 'ar-EG' : 'en-US', { day: 'numeric', month: 'long', year: 'numeric' })} · {toAr(g.items.length)}</div>
                  </div>
                </div>
                    ...
                    return (
                      <button key={m.id} onClick={() => m.contact_id && navigate(`/leads?highlight=${m.contact_id}`)}
                        ...
                        <div className="flex flex-col items-center justify-center min-w-[46px] ps-1">
                          ...
                          <span className="text-[10px] font-bold text-content-muted dark:text-content-muted-dark mt-0.5">{ap}</span>
                        </div>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {m.notes && <div className="flex items-center gap-1 text-[11px] text-content-muted dark:text-content-muted-dark mt-1 truncate"><MapPin size={11} className="shrink-0" />{m.notes}</div>}
                          {m.result && <div className="flex items-start gap-1.5 text-[11.5px] text-content dark:text-content-dark mt-1"><MessageSquare size={12} className="shrink-0 mt-0.5 text-content-muted dark:text-content-muted-dark" /><span className="truncate">{m.result}</span></div>}
                        </div>
            ...
            {meetings.length >= FETCH_LIMIT && (
              <p className="text-center text-[11.5px] text-content-muted dark:text-content-muted-dark pt-2">{isRTL ? `يُعرض أحدث ${toAr(FETCH_LIMIT)} اجتماع` : `Showing the latest ${FETCH_LIMIT}`}</p>
            )}
          </div>
        )}
      </div>
