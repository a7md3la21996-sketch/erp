...
export default function ReassignmentsTab({ lang, isRTL }) {
  ...
  const kpis = [
    { label: isRTL ? 'إجمالي التوزيعات' : 'Total moves', value: totalMoves, sub: isRTL ? 'إعادة إسناد في الفترة' : 'reassignments in period', icon: ArrowRightLeft, color: 'brand' },
    { label: isRTL ? 'موظفين استلموا' : 'Received by', value: rows.filter(r => r.received > 0).length, sub: isRTL ? 'عدد الموظفين' : 'agents', icon: UserPlus, color: 'emerald' },
    { label: isRTL ? 'الأكثر توزيعاً' : 'Top distributor', value: topActor?.count || 0, sub: topActor?.name || '—', icon: UserMinus, color: 'amber' },
  ];
  ...
  return (
    <div dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-4">
        {kpis.map((k, i) => (
          <MetricCard key={i} icon={k.icon} label={k.label} value={k.value} sublabel={k.sub} color={k.color} />
        ))}
      </div>
