...
export default function TargetTrackerPage() {
  ...
  const c = {
    bg: isDark ? '#152232' : '#f1f5f9', cardBg: isDark ? '#1a2234' : '#ffffff',
    border: isDark ? 'rgba(74,122,171,0.2)' : '#e2e8f0', text: isDark ? '#E2EAF4' : '#0f172a',
    textMuted: isDark ? '#8BA8C8' : '#64748b', inputBg: isDark ? '#0F1E2D' : '#f8fafc',
    thBg: isDark ? 'rgba(74,122,171,0.08)' : '#f8fafc', rowHover: isDark ? 'rgba(74,122,171,0.08)' : '#f1f5f9',
    primary: '#2B4C6F', accent: '#4A7AAB', light: '#6B8DB5',
  };
    ...
    <div style={{ padding: 24, direction: isRTL ? 'rtl' : 'ltr', background: c.bg, minHeight: '100vh' }}>
      <div style={{ marginBottom: 24, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: c.textMuted, margin: '4px 0 0' }}>{lang === 'ar' ? 'أداء فريق المبيعات الشهري' : 'Monthly Sales Team Performance'}</p>
        </div>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {MONTHS.map(m => (
            <button key={m.id} onClick={() => setSelectedMonth(m.id)} style={{
              ...
              color: selectedMonth === m.id ? '#fff' : c.textMuted, cursor: 'pointer', transition: 'all 0.15s',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 16, marginBottom: 24 }}>
        {[
          ...
          <div key={i} style={{ background: c.cardBg, borderRadius: 12, padding: '16px 20px', border: `1px solid ${c.border}` }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>{kpi.icon}<span style={{ fontSize: 12, color: c.textMuted, fontWeight: 500 }}>{kpi.label}</span></div>
          ...
          <div style={{ padding: '14px 20px', borderBottom: `1px solid ${c.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {[{ key: 'pct', ar: 'بالنسبة', en: '% Target' }, { key: 'achieved', ar: 'بالمبلغ', en: 'Amount' }, { key: 'deals', ar: 'بالصفقات', en: 'Deals' }].map(s => (
                <button key={s.key} onClick={() => setSortBy(s.key)} style={{ padding: '5px 12px', borderRadius: 6, fontSize: 11, fontWeight: 600, border: `1px solid ${sortBy === s.key ? c.accent : c.border}`, background: sortBy === s.key ? c.accent : 'transparent', color: sortBy === s.key ? '#fff' : c.textMuted, cursor: 'pointer' }}>{lang === 'ar' ? s.ar : s.en}</button>
              ))}
            </div>
          </div>
            ...
            <thead>
              <tr style={{ background: c.thBg }}>
                {[{ ar: '#', en: '#', w: 40 }, { ar: 'الموظف', en: 'Agent' }, { ar: 'التارجت', en: 'Target', w: 100 }, { ar: 'المحقق', en: 'Achieved', w: 100 }, { ar: 'النسبة', en: '% Done', w: 140 }, { ar: 'الصفقات', en: 'Deals', w: 70 }, { ar: 'التغيير', en: 'vs Last', w: 80 }].map((h, i) => (
                  <th key={i} style={{ padding: '10px 14px', fontSize: 11, fontWeight: 600, color: c.textMuted, textAlign: isRTL ? 'right' : 'left', whiteSpace: 'nowrap', width: h.w || undefined }}>{lang === 'ar' ? h.ar : h.en}</th>
                ))}
              </tr>
            </thead>
            <tbody>
                    ...
                    <td style={{ padding: '12px 14px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: 11, color: c.textMuted, marginTop: 1 }}>{emp.role === 'sales_director' ? (lang === 'ar' ? 'مدير مبيعات' : 'Sales Director') : emp.role === 'sales_manager' ? (lang === 'ar' ? 'مدير فريق' : 'Sales Manager') : emp.role === 'team_leader' ? (lang === 'ar' ? 'قائد فريق' : 'Team Leader') : (lang === 'ar' ? 'موظف مبيعات' : 'Sales Agent')}</div>
                        </div>
                      </div>
                    </td>
                    <td style={{ padding: '12px 14px', fontSize: 13, color: c.textMuted, fontWeight: 500 }}>{fmt(emp.target)}</td>
              ...
              {[1, 0, 2].map((idx) => {
                ...
                return (
                  <div key={idx} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', width: 80 }}>
                    ...
                    <div style={{ fontSize: 10, color: c.textMuted, marginBottom: 4, textAlign: 'center' }}>{lang === 'ar' ? emp.full_name_ar.split(' ')[0] : emp.full_name_en.split(' ')[0]}</div>
            ...
            {['mar', 'feb', 'jan'].map((m) => {
              ...
              return (
                <div key={m} style={{ marginBottom: 10 }}>
                  ...
                  <div style={{ height: 6, borderRadius: 4, background: isDark ? 'rgba(74,122,171,0.12)' : '#e2e8f0' }}><div style={{ height: '100%', borderRadius: 4, width: `${Math.min(mPct,100)}%`, background: m === selectedMonth ? c.accent : c.light }} /></div>
                </div>
              );
            })}
          </div>
