...
export default function Header({ onMenuClick }) {
  ...
  useEffect(() => {
    const refresh = () => { getUnreadCount(profile?.id).then(c => setUnreadCount(c)).catch(() => {}); };
    refresh();
  ...
  useEffect(() => {
    if (!showNotifications) {
      getUnreadCount(profile?.id).then(c => setUnreadCount(c)).catch(() => {});
    }
  }, [showNotifications]);
        ...
        {isRealAdmin && (
          <div ref={roleSwitcherRef} className="relative">
            ...
            {showRoleSwitcher && (
              <div
                dir={isRTL ? 'rtl' : 'ltr'}
                className="fixed top-14 end-2 w-[220px] max-w-[calc(100vw-16px)] rounded-xl bg-surface-card dark:bg-surface-card-dark border border-edge dark:border-edge-dark shadow-lg dark:shadow-2xl py-1.5 z-[100]"
              >
