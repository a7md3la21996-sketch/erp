-- ============================================================================
-- Follow-up link, Phase 1 — record which activity a follow-up came from.
--
-- Adds tasks.source_activity_id (the activity that spawned the follow-up) and
-- makes log_interaction set it at creation, so the drawer can JOIN on a real
-- link instead of guessing by timestamp. Backfill for old rows is a separate
-- one-time step. Additive + reversible (DROP COLUMN).
-- ============================================================================
